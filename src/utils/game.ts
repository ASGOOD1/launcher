...
export const startGame = async (
  ...
  try {
    // Afișăm un modal inițial pentru ca utilizatorul să știe că se descarcă modpack-ul
    showMessageBox({
      title: "Descărcare Modpack",
      description: "Se verifică fișierele...",
      buttons: [], // Fără butoane pentru a preveni închiderea accidentală în timpul descărcării
    });
    ...
    const unlisten = await listen<ModpackProgressPayload>("modpack-progress", (event) => {
      ...
      Log.debug(`Modpack (${percentage}%): descarc ${file} ${downloaded_mb}MB / ${total_mb}MB`);

      // Actualizăm modalul dinamic cu megabiții descărcați și procentul real
      showMessageBox({
        title: `Descărcare Modpack (${percentage}%)`,
        description: `Se descarcă arhiva: ${downloaded_mb} MB / ${total_mb} MB`,
        buttons: [],
      });
    });
    ...
    await invoke("sync_modpack", {
      gtaPath: gtasaPath,
      manifestUrl: "https://asgood.ro/modpack/manifest.json",
    });

    unlisten();
    hideMessageBox();
  }  catch (e) {
    Log.debug("Eroare la sincronizarea modpack-ului:", e);
    showMessageBox({
      title: "Eroare Modpack",
      description: `Eroare exactă: ${e}`,
      buttons: [{ title: t("close"), onPress: hideMessageBox }],
    });
    return;
  }
