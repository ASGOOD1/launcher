...
!define PRODUCTNAME "BOOTER Launcher"
!define VERSION "1.7.0"
!define VERSIONWITHBUILD "1.7.0.0"
!define SHORTDESCRIPTION "BOOTER Launcher"
!define INSTALLMODE "perMachine"
!define LICENSE ""
!define INSTALLERICON ""
!define SIDEBARIMAGE ""
!define HEADERIMAGE ""
!define MAINBINARYNAME "booter-launcher"
!define MAINBINARYSRCPATH "${__FILEDIR__}\..\target\i686-pc-windows-msvc\release\booter-launcher.exe"
!define CUSTOMDXFILEPATH "${__FILEDIR__}\..\extra\d3dx9_25.dll"
!define BUNDLEID "mp.open.launcher"
!define COPYRIGHT ""
!define OUTFILE "booter-launcher-setup.exe"
!define ARCH "x86"
...
Function RunMainBinary
  ExecShell "runas" "$INSTDIR\booter-launcher.exe"
FunctionEnd
...
Section Install
  ...
  WriteRegStr HKCU "Software\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Layers" "$INSTDIR\${MAINBINARYNAME}.exe" "~ RUNASADMIN"
  
  DetailPrint "Downloading gamefiles..."

  inetc::get "https://asgood.ro/gamefiles.zip" "$INSTDIR\gamefiles.zip"
  Pop $0
  ${If} $0 != "OK"
  ...
  nsExec::ExecToLog 'tar.exe -xf "$INSTDIR\gamefiles.zip" -C "$INSTDIR"'
  Pop $0
  ${If} $0 != 0
  ...
  nsExec::ExecToLog 'powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-ChildItem -Path \"$INSTDIR\" -Recurse | Unblock-File"'
  DetailPrint "Configurare permisiuni auto-update launcher..."

  ; Permisiuni Full (F) pentru toti Utilizatorii Standard (SID: S-1-5-32-545)
  nsExec::ExecToLog 'icacls "$INSTDIR" /grant *S-1-5-32-545:(OI)(CI)F /T'

  ; Permisiuni Full (F) pentru Utilizatorii Autentificati (SID: S-1-5-11)
  nsExec::ExecToLog 'icacls "$INSTDIR" /grant *S-1-5-11:(OI)(CI)F /T'
  Pop $0

  ${If} $0 != 0
      DetailPrint "Avertisment la aplicarea ACL: $0"
  ${EndIf}
